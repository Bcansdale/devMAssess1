...
- [ ] **D.** There is no error; the code generates the expected output.
<!-- i <= nums.lenght will result in an error due to iterating outside of the array> -->
...
```
console.log outputs to the console. return sends the output back to the function
```

**_What's the difference between defining and calling a function?_**

```
You define a function to give it a name to "Call or invoke" and to make an actual function. When you call a function you are telling it to run. There is no point in having a function if you do not call it.
```
...
```
A string represents a set of 'words' in programming. 
```

**_Number_**

```
A number represents an actual number to the computer that can be ussed for math operations.
```
...
```
An array is a form of data set that can be used to pull data using statements or functions. 
```
